...
vignette: >
  %\VignetteIndexEntry{SBG Creating a Workflow}
...
To run this workflow, we need three apps in our project:
* get-http_file
...
Add app "get-http_file" 
```{r}
a$copyApp(id =  "tsang/temp/txfr/0", project = pid, name = "copy-get-http-fileTool")
## check it is copied
a$app(project = pid)
```


Search for the PUBLIC workflow RNA-Seq Alignment STAR
```{r}
a$app("STAR", visibility = "public", complete = TRUE)

a$copyApp(id =  "djordje_klisic/public-apps-by-seven-bridges/rna-seq-alignment-star/3", project = pid, name = "rna-seq-star-workflow-copy")
## check it is copied
a$app(project = pid)
```
...
To run the App we need to create a draft Task.
...
```{r}
run_getfl <-"Durga/irp-cgc-with-r/copy-get-http-fileTool/0"

a$project("IRP-CGC-with-R")$task_add(name = "Draft GetS3Files", 
...
## confirm
a$project("IRP-CGC-with-R")$task(status = "draft")
```
...
```{r}
tsk1 <- a$project("IRP-CGC-with-R")$task(status = "draft")
tsk1$run()
...
```{r}
run_STARwfl <-"Durga/irp-cgc-with-r/rna-seq-star-workflow-copy/0"

a$project("IRP-CGC-with-R")$task_add(name = "Draft GetS3Files", 
                           description = "Using get_http_file app to get files from S3 bucket", 
                           app = run_STARwfl,
                           inputs = list(url = , ofname = ))
