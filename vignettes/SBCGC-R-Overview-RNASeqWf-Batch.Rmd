...
Now let's create a new project called "IRP-CGC-Workshop2", save it to a 'p' object for convenient usage for any call related to this project.

```{r}
(p <- a$project_new("IRP-CGC-Workshop2", 
                    billing_group_id = b$id, 
                    description = "This project is from IRP-CGC Workshop at CBIIT"))
```
...
```{r}
## remove it, not run
a$project("IRP-CGC-Workshop2")$delete()
## check
## will delete all projects matcht the name
delete(a$project("IRP-CGC-Workshop2_donnot_delete_me"))
```
...
```{r}
## Delete file
a$project("IRP-CGC-Workshop2")$file()[[1]]$delete()
## confirm the deletion
a$project("IRP-CGC-Workshop2")$file()

## Search for specific file by pattern match and delete, this can delete multiple files too.
a$project("IRP-CGC-Workshop2")$file("Readspergene")$delete()

```

## Lets Copy some Apps into our Project IRP-CGC-Workshop2
...
## BATCH - Running the RNA-Seq Alignment STAR which takes multiple BAM files as input
```{r}
## Copy the app
