...
### 1) Creating a Docker image of STAR (Optional)
...
Test the WDL file locally -  
Creating JSON file
```{sh, eval=FALSE}
java -jar /Users/<path file>/wdltool-0.5.jar inputs StarAlign_cfg2.wdl > StarAlign_cfg2.json
```

Modifying the json file to include the location/names of the input files.  
```{}
{
...
If the run is successfully completed, you are ready to run this WDL file on the FireCloud.
...
###3) Set up STAR alignment run on the FireCloud - for detailed description of each step, please refer to [FireCloud intro](https://github.com/teamcgc/cgcR/blob/master/vignettes/Firecloud.intro.Rmd)  
...
###4) Visualize the alignment using IGV (optional)
